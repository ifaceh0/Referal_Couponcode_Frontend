...
import { signupEmployee } from "../../../api/signin";
...
const ShopEmployee = () => {
  ...
  try {
    ...
    const responseMessage = await signupEmployee(submitData, invitationToken);
  ...
  const handlePopupSubmit = async () => {
    ...
    try {
      const { confirmPassword, ...submitData } = formData;
      const responseMessage = await signupEmployee(
        submitData,invitationToken
      );

      toast.update(toastId, {
        render: responseMessage || "Signup successful!...",
        type: "success",
        autoClose: 5000,
        isLoading: false,
      });
