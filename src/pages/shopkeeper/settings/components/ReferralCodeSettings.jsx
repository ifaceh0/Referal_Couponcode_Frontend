...
// import { discountData } from "../../../../utils/demoData";
// import { toast } from 'react-toastify';
// import { updateAndSaveSettingAction, getSettingsAction } from "../../../../api/settingPageApi";
...
//     const handleSaveCouponPromotion = () => {
//         setIsEditingCouponPromotion(false);
//         toast.success('Coupon promotion settings saved');
//     };
...
//     const applyReward = (customerType) => {
//         const amount = customerType === 'existing' ? existingCustomerValue : 
//                       (useCredits ? signupDollars : signupPoints);
//         const unit = useCredits ? 'dollars' : 'points';
        
//         toast.success(`Applied ${amount} ${unit} to ${customerType} customer`);
//     };

//     const handleSendPromotion = async () => {
//         try {
//             if (!promotion.beginDate || !promotion.expiryDate || !promotion.referralAmount || !promotion.referrerAmount) {
//                 toast.error('Please fill all promotion fields before sending');
//                 return;
//             }

//             if (!isFutureDate(promotion.beginDate)) {
//                 toast.error('Begin date must be in the future!');
//                 return;
//             }

//             if (!isValidDateRange(promotion.beginDate, promotion.expiryDate)) {
//                 toast.error('Expiry date must be after begin date!');
//                 return;
//             }

//             const response = await updateAndSaveSettingAction({
//                 shopkeeperId,
//                 promotion: {
//                     ...promotion,
//                     currencyType: useCredits ? 'dollars' : 'points'
...
//             if (!response.success) {
//                 throw new Error(response.message || 'Failed to send promotion');
//             }

//             toast.success('Referral promotion sent successfully!');
...
//         } catch (error) {
//             console.error('Error sending referral promotion:', error);
//             toast.error(error.message || 'Failed to send referral promotion. Please try again.');
//         }
//     };
...
//         try {
//             if (!couponPromotion.beginDate || !couponPromotion.expiryDate || !couponPromotion.couponAmount || !couponPromotion.limitOfUse) {
//                 toast.error('Please fill all coupon promotion fields before sending');
//                 return;
//             }

//             if (!isFutureDate(couponPromotion.beginDate)) {
//                 toast.error('Begin date must be in the future!');
//                 return;
//             }

//             if (!isValidDateRange(couponPromotion.beginDate, couponPromotion.expiryDate)) {
//                 toast.error('Expiry date must be after begin date!');
//                 return;
//             }
...
//             if (!response.success) {
//                 throw new Error(response.message || 'Failed to send coupon promotion');
//             }

//             toast.success('Coupon promotion sent successfully!');
//         } catch (error) {
//             console.error('Error sending coupon promotion:', error);
//             toast.error(error.message || 'Failed to send coupon promotion. Please try again.');
//         }
//     };
...
//     if (loading) {
//         return <div className="p-6 max-w-4xl mx-auto bg-white shadow-md rounded-lg">Loading...</div>;
//     }
