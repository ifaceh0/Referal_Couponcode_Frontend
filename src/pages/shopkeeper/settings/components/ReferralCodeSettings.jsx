            ...
            try {
                ...
                if (user?.id) {
                    ...
                    if (response) {
                        ...
                        setMilestoneMapping(
                        settings.milestoneRewards?.length > 0
                            ? settings.milestoneRewards.map((m) => ({
                                milestoneType: "Referrals",
                                milestoneValue: m.thresholdRefer?.toString() || "",
                                rewardType: "Cash",
                                reward: m.amount?.toString() || "",
                                thresholdRefer: m.thresholdRefer,
                                amount: m.amount,
                            }))
                            : discountData.milestoneMapping
                        );
                    }
                }
            } catch (error) {
        ...
        try {
            ...
            const settingsData = {
                ...
                milestoneRewards: milestoneMapping
                    .filter(m => m.milestoneValue && m.reward)
                    .map(m => ({
                    thresholdRefer: Number(m.thresholdRefer || m.milestoneValue),
                    amount: Number(m.amount || m.reward),
                    })),
            };
